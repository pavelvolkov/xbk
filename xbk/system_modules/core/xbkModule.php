...
class xbkModule extends xbkContextObject
{
    ...
    protected function loadFromDb ($moduleName)
    {    	global $CONFIG;
    	...
    	if (count($found) > 0)
    	{
    	    try {
        		...
        		// Общая информация
        		$this->meta['project'] = $Module_Record->project;
        		$this->meta['version'] = $Module_Record->version;
        		$this->meta['xbk_version'] = $Module_Record->xbk_version;
        		$this->meta['migration'] = $Module_Record->migration_required;
        		$this->meta['class'] = $Module_Record->class;
        		$this->meta['dependencies'] = $Module_Record->dependencies;
    	...
    	if (xbkFunctions::moduleFileExists($moduleName)) {
            ...
            if ($parser_n == 1) {
                ...
                if ($metaDOM->item(0)->getElementsByTagName('project')->length > 0) {
                    $this->meta['project'] = $metaDOM->item(0)->getElementsByTagName('project')->item(0)->nodeValue;
                } else $this->meta['project'] = null;
                if ($metaDOM->item(0)->getElementsByTagName('version')->length > 0) {
                    $this->meta['version'] = $metaDOM->item(0)->getElementsByTagName('version')->item(0)->nodeValue;
                } else $this->meta['version'] = null;
                if ($metaDOM->item(0)->getElementsByTagName('xbkVersion')->length > 0) {
                    $this->meta['xbk_version'] = $metaDOM->item(0)->getElementsByTagName('xbkVersion')->item(0)->nodeValue;
                } else $this->meta['xbk_version'] = null;
                if ($metaDOM->item(0)->getElementsByTagName('migration')->length > 0) {
                    $this->meta['migration'] = $metaDOM->item(0)->getElementsByTagName('migration')->item(0)->nodeValue;
                }
                if ($metaDOM->item(0)->getElementsByTagName('class')->length > 0) {
                    $this->meta['class'] = $metaDOM->item(0)->getElementsByTagName('class')->item(0)->nodeValue;
                } else $this->meta['class'] = null;
                if ($metaDOM->item(0)->getElementsByTagName('dependencies')->length > 0) {
                    $this->meta['dependencies'] = $metaDOM->item(0)->getElementsByTagName('dependencies')->item(0)->nodeValue;
                } else $this->meta['dependencies'] = null;

            } else if ($parser_n == 0) {
                $abstractDOM = $declarationDOM->getElementsByTagName('abstract');
                if ($abstractDOM->item(0)->getElementsByTagName('name')->length > 0)
                {
                    $this->meta['project'] = $abstractDOM->item(0)->getElementsByTagName('name')->item(0)->nodeValue;
                } else $this->meta['name'] = null;
                if ($abstractDOM->item(0)->getElementsByTagName('version')->length > 0)
                {
                    $this->meta['version'] = $abstractDOM->item(0)->getElementsByTagName('version')->item(0)->nodeValue;
                } else $this->meta['version'] = null;
                if ($abstractDOM->item(0)->getElementsByTagName('xbkVersion')->length > 0)
                {
                    $this->meta['xbk_version'] = $abstractDOM->item(0)->getElementsByTagName('xbkVersion')->item(0)->nodeValue;
                } else $this->meta['xbk_version'] = null;
                if ($abstractDOM->item(0)->getElementsByTagName('migration')->length > 0)
                {
                    $this->meta['migration'] = $abstractDOM->item(0)->getElementsByTagName('migration')->item(0)->nodeValue;
                }
                if ($abstractDOM->item(0)->getElementsByTagName('infoClass')->length > 0)
                {
                    $this->meta['class'] = $abstractDOM->item(0)->getElementsByTagName('infoClass')->item(0)->nodeValue;
                } else $this->meta['class'] = null;
    ...
    public function prepareInfo ()
    {
    	if ($this->Info == null)
    	{
            $this->Info =& $this->factory($this->meta['class']);
        }
    }
    ...
    {    	if (isset($this->meta['migration'])) return (int)$this->meta['migration'];
    	else return null;
    }
