...
class xbkSuperadminModules extends xbkSection
{
    ...
    public function moduleToArray (&$Module)
    {    	// Ссылка на подробную информацию о модуле
        ...
        $meta = $Module->getMeta();
        $result['name'] = $meta['project'];
        $result['version'] = $meta['version'];
        $result['xbk_version'] = $meta['xbk_version'];
        $result['info_link'] = $InfoUri->build('info='.$meta['project']);
        $result['action_link'] = $InfoUri->build('info='.$meta['project']);
        $result['status_link'] = $InfoUri->build('info='.$meta['project']);
    ...
    public function moduleInfo (&$Module)
    {    	if ($this->installHandling($Module)) return;
    	...
    	if (isset($meta['version']) ? $meta['version'] != null : false)
    	{
    		array_push($rows,
        		Array(
    	...
    	if (isset($meta['xbk_version']) ? $meta['xbk_version'] != null : false)
    	{
    		array_push($rows,
        		Array(
            		'name' => $this->_LANG['superadmin_module_xbk_version'],
            		'value' => $meta['xbk_version']
        		)
    		);
    	}
    	...
    	if ($this->coreInstalled)
    	{    		// Получаем данные о текущей миграции
    		if (isset($meta['migration']))
    		{
        		$migration_required = $meta['migration'];
